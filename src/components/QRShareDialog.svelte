<script lang="ts">
	...
	import { copyShareLink, getTwitterShareURL, getTelegramShareURL } from '$lib/social/twitter';
	import { getActiveDomain } from '$lib/domain/rotation';
	import { THEME } from '$lib/constants';
	import { hapticFeedback } from '$lib/tg/miniapp';
	import { isTGMiniApp } from '$lib/tg/miniapp';
	 ...
	 * 分享到 Telegram
	 */
	function handleShareTelegram(): void {
		const url = getTelegramShareURL(video);
		window.open(url, '_blank', 'width=600,height=400');
		if (isTGMiniApp()) hapticFeedback('light');
	}
