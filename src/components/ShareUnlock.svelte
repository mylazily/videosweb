	...
	function handleShare(platform: string) {
		...
		switch (platform) {
			case 'twitter':
				...
				break;
			case 'telegram':
				window.open(`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent('推荐一部好片')}`);
				break;
		}
	...
	>
		...
		<div class="flex gap-3">
			...
			<button
				...
				class="flex-1 py-2.5 bg-[#0088cc] text-white rounded-lg text-xs font-medium btn-press flex items-center justify-center gap-1.5"
				disabled={sharing}
			>
				<svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
