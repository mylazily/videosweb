<script lang="ts">
  ...
  async function loadMore() {
    ...
    try {
      ...
      const res = await fetch(
        `${base}/api/v1/videos?category=${slug}&page=${pageNum}&page_size=10`,
      ...
      if (res.ok) {
        ...
        const moreVideos: Video[] = resData.data?.list || resData.data || [];
  ...
  onMount(() => {
    if (!data.videos?.length) loadMore();
...
<svelte:head>
  <title>{category?.name || '分类'} - 影视库</title>
  <meta name="description" content="{category?.name || '分类'}影视推荐，高清在线观看" />
</svelte:head>
    ...
    <div class="px-3 mt-2">
      ...
      {:else if videos.length === 0}
        ...
        <div class="flex flex-col items-center justify-center py-20" style="animation: fadeIn 0.3s ease-out;">
          ...
          <p class="text-gray-400 text-sm">暂无该分类内容</p>
        </div>
      {:else}
        <!-- 视频网格 -->
        <div class="grid grid-cols-2 gap-3">
